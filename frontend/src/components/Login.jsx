...
import { setUser } from '../state/UserToken';
...
const Login = () => {
    ...
    const onFinish = async (values) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_HOST}/api/user/login`, {
                method: 'POST',
                headers: {
            ...
            console.log(data)
            dispatch(setUser({token:data.data.token,role:data.data.role}))
    ...
    const onFinishFailed = (errorInfo) => {
        setAlerts([...alerts, { type: 'error', message: 'Login failed. Please check your credentials.' }]);
    };
                        ...
                        >
                            ...
                            <div className="text-center mt-4">
                                <p className="text-gray-600">
                                    Don't have an account? <Link to="/user/register" className="text-secondary1 font-semibold">Register</Link>
                                </p>
                            </div>
                        </Form>
                    </Spin>
